...
protected:
    ...
    void SetupDefaultBehaviors() {
        ...
        ON_CALL(*mock_ws_server_, Stop())
            .WillByDefault(Invoke([this]() {
                server_running_ = false;
            }));

        // Mock P2P network behaviors
        ON_CALL(*mock_p2p_network_, Initialize(_))
            .WillByDefault(Return(true));

        ON_CALL(*mock_p2p_network_, Connect(_, _))
            .WillByDefault(Return(true));
    }

    // Helper to simulate complete room creation flow
    void SimulateRoomCreation(const std::string& room_name, const std::string& game_id) {
        ...
        mock_ws_server_->SimulateClientMessage("host-client", create_response.dump());
    }

    // Helper to simulate player joining
    void SimulatePlayerJoin(const std::string& player_id, const std::string& nickname) {
        ...
        mock_ws_server_->SimulateClientMessage("host-client", join_notification.dump());
    }
...
TEST_F(ModelAIntegrationTest, CompleteRoomCreationFlow) {
    ...
    room_client_->SetOnConnected([&connect_promise]() {
    ...
    room_client_->SetOnRoomCreated([&room_id_promise](const std::string& room_id) {
    ...
    room_client_->SetOnPlayerJoined([&joined_players, &players_mutex](const PlayerInfo& player) {
    ...
    // Verify all players joined
    {
        ...
        EXPECT_THAT(joined_players, UnorderedElementsAre("player1", "player2", "player3"));
    }
...
TEST_F(ModelAIntegrationTest, P2PConnectionEstablishment) {
    ...
    // Initialize host
    ASSERT_EQ(ErrorCode::Success, host_p2p->Initialize("host-peer-id", p2p_config));
    
    // Initialize client
    ASSERT_EQ(ErrorCode::Success, client_p2p->Initialize("client-peer-id", p2p_config));
    ...
    host_p2p->SetOnPeerConnected([&host_connected_promise](const std::string& peer_id) {
        if (peer_id == "client-peer-id") {
            host_connected_promise.set_value(true);
        }
    });
    
    client_p2p->SetOnPeerConnected([&client_connected_promise](const std::string& peer_id) {
    ...
    host_p2p->SetOnDataReceived(
        [&data_received_promise](const std::string& peer_id, const std::vector<uint8_t>& data) {
            if (peer_id == "client-peer-id") {
    ...
    ASSERT_EQ(ErrorCode::Success, client_p2p->SendData("host-peer-id", test_data));
...
TEST_F(ModelAIntegrationTest, RelayFallbackMechanism) {
    ...
    ASSERT_EQ(ErrorCode::Success, p2p_network->Initialize("test-peer", p2p_config));
    ...
    relay_client->SetOnConnected([&relay_activated]() {
        relay_activated = true;
    });
    
    // Attempt P2P connection
    ASSERT_EQ(ErrorCode::Success, 
              p2p_network->ConnectToPeer("remote-peer", "/ip4/10.0.0.1/tcp/4001"));
...
TEST_F(ModelAIntegrationTest, NetworkResilienceAndReconnection) {
    ...
    room_client_->SetOnDisconnected([&disconnect_count, &event_mutex](const std::string& reason) {
        std::lock_guard<std::mutex> lock(event_mutex);
        disconnect_count++;
    });
...
TEST_F(ModelAIntegrationTest, ConcurrentRoomOperations) {
    ...
    for (int i = 0; i < num_clients; ++i) {
        futures.push_back(std::async(std::launch::async, [&clients, i, &room_ids, &room_ids_mutex]() {
            RoomInfo room_info;
            room_info.room_name = "Room " + std::to_string(i);
            room_info.game_id = "0100000000010000";
            room_info.max_players = 4;
            
            clients[i]->SetOnRoomCreated([&room_ids, &room_ids_mutex](const std::string& room_id) {
