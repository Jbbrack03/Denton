 ...
 * Provides configurable exponential backoff with jitter and maximum delay limits
 */
 ...
 * Handles connection to room server, message serialization, and reconnection logic
 */
class RoomClient {
public:
    ...
    void SetOnDisconnectedCallback(std::function<void(const std::string&)> callback);
...
private:
    ...
    void DispatchMessage(const BaseMessage& base_message, const std::string& raw_message);
